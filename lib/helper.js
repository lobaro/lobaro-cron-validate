...
const checkHashedElement = (element, cronFieldType, options) => {
    ...
    if (element[0] !== "H") {
        return (0, result_1.err)("Hashed value doesn't begin field");
    }
    // Simplest case
    if (element === "H") {
        return (0, result_1.valid)(true);
    ...
    if (stepArray.length > 2) {
        return (0, result_1.err)(`Hashed element '${element}' is not valid. (More than one '/')`);
    }
    ...
    if (stepArray.length === 2) {
        ...
        if (!secondStepElement) {
            return (0, result_1.err)(`Second step element '${secondStepElement}' of '${element}' is not valid (doesnt exist).`);
        }
        if (isNaN(Number(secondStepElement))) {
            return (0, result_1.err)(`Second step element '${secondStepElement}' of '${element}' is not valid (not a number).`);
        }
        if (Number(secondStepElement) === 0) {
            return (0, result_1.err)(`Second step element '${secondStepElement}' of '${element}' cannot be zero.`);
        }
    ...
    if (rangeMatch === null) {
        return (0, result_1.err)(`Hashed element ${element} doesn't contain a valid iterator range.`);
    }
    ...
    if (Number(rangeMatch[1]) > Number(rangeMatch[2])) {
        return (0, result_1.err)(`Lower range end '${rangeMatch[1]}' is bigger than upper range end '${rangeMatch[2]}' of ${cronFieldType} field.`);
    }
    return (0, result_1.valid)(true);
};
